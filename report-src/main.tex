...
\begin{abstract}
  ...
  and demonstrate that the PCAF's structured intervention yields a significant uplift in final accuracy in comparison to the MathArena's baseline (51.3\% versus 39.6\%), with a 37.4\% recovery rate confirming the efficacy of deterministic, architecture-driven reflection over traditional self-correction techniques.
\end{abstract}
...
\begin{table}[h]
  ...
  \begin{tabular}{lcccc}
    \toprule
    \textbf{Dataset} & \textbf{Baseline} & \textbf{PCAF Accuracy} & \textbf{Avg. Turns} & \textbf{Recovery Rate} \\
    \midrule
    AIME 2025 (Subset)      & 50.0\% \\ & 56.7\% & 2.16 & 38.2\%
    HMMT Feb '25 (Subset)   & 29.17\% \\ & 45.8\% & 2.47 & 36.6\%
    \bottomrule
  \end{tabular}
\end{table}
