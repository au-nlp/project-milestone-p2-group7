...
\begin{abstract}
  ...
  and demonstrate that the PCAF's structured intervention yields a significant uplift in final accuracy (51.3\% weighted average), with a 37.4\% recovery rate confirming the efficacy of deterministic, architecture-driven reflection over traditional self-correction techniques.
\end{abstract}
...
To evaluate the efficacy of the Prompt-Based Collaborative Agent Framework (PCAF), we benchmarked the system on a subset of high-difficulty problems from the MathArena benchmark (AIME 2025 and HMMT February 2025). These datasets were selected for their complexity, often requiring multi-step reasoning that challenges standard Zero-Shot Chain-of-Thought (CoT) approaches.
...
Our experiments on the MathArena benchmark demonstrate that the PCAF architecture yields a quantifiable improvement in reliability compared to standard one-shot approaches. By aggregating performance across multiple independent runs, we observe that the system's collaborative loop effectively converts initial reasoning errors into successful solutions.
...
\begin{table}[h]
  ...
  \begin{tabular}{lcccc}
    \toprule
    \textbf{Dataset} & \textbf{PCAF Accuracy} & \textbf{Avg. Turns} & \textbf{Recovery Rate} & \textbf{Baseline (Est.)} \\
    \midrule
    AIME 2025 (Subset)      & 56.7\% & 2.16 & 38.2\% & $\approx$ 25.0\% \\
    HMMT Feb '25 (Subset)   & 45.8\% & 2.47 & 36.6\% & $\approx$ 20.0\% \\
    \midrule
    \textbf{Weighted Average} & \textbf{51.3\%} & \textbf{2.31} & \textbf{37.4\%} & \textbf{22.5\%} \\
    \bottomrule
  \end{tabular}
\end{table}
