...
\author{%
  ...
  Department of Computer Science\\
  Aarhus University\\
  \texttt{au803262@uni.au.dk} \\
  \And
  András Szabolcs Gyüre \\
  Department of Computer Science\\
  Aarhus University\\
  \texttt{au803280@uni.au.dk} \\
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{PCAF_System_Architecture.png}
    \caption{PCAF System Architecture}
    \label{fig:pcaf_architecture}
\end{figure}
...
\textbf{Implementation.} The framework utilizes the \texttt{DeepSeek-V3-03-24} model hosted on Azure OpenAI. The architecture consists of three specialized agents. The Solver, the Verifier and the Planner, all operating within a single LLM context. A key component is the Solver's \texttt{PersistentSolverSandbox}, which maintains variable state across turns to allow for iterative code refinement.
...
While the system excelled in Algebra and Number Theory, qualitative analysis of failures reveals a weakness in Geometry. When the Solver attempted to parameterize complex shapes (e.g., AIME 2025 Problem 2) in Python, it often struggled to translate visual constraints into correct coordinate code. The Verifier correctly flagged inconsistencies, but the Planner lacked a visual modality to offer spatial correction, leading to loop exhaustion. This suggests that for geometric reasoning, the text based code sandbox must be augmented with Vision Language Models (VLMs).

\section{Conclusion}
The results of this study validate the PCAF as a successful architectural intervention for complex mathematical reasoning. By transitioning from a linear, single inference process to a modular Solver, Verifier, Planner loop, we have shown that the hallucination and stubbornness inherent in LLMs are not immutable traits, but can be managed through structured internal collaboration.

The primary success of the PCAF lies in its ability to enforce Grounding. The high recovery rate observed in our experiments confirms that the framework’s deterministic correction logic effectively rescues solutions that would otherwise fail under standard conditions.

 While the observed limitations in Geometry suggest that a purely text-based sandbox may eventually require visual-modality integration, the current PCAF provides a reliable and transparent blueprint for creating more trustworthy autonomous agents. Ultimately, this work proves that for high-stakes reasoning tasks, a model's architecture is as critical as its scale. 
...
\newpage
\appendix
% 1. Force Section headers to say "Appendix A"
\renewcommand{\thesection}{Appendix \Alph{section}}
% 2. Force Subsection headers to just say "A.1" (Clean)
...
\section{Failure Case Study: Geometry (The ``Blind Spot'')}
\label{app:geometry_failure}

\textit{While PCAF excels at Algebra, this case study from the AIME 2025 dataset (Problem 2) illustrates a structural limitation. In Run 2, the Solver hallucinated a spatial constraint that the text-based Verifier could only catch via logical deduction, not visual inspection.}

\noindent\hrulefill

\textbf{Problem (AIME I 2025, Problem 2):} Points $M$ and $N$ lie on sides $AB$ and $AC$ of $\triangle ABC$. A heptagon is formed by removing specific triangles. Given the area of $\triangle ABC$ is 588, determine the area of the heptagon.

\vspace{0.5em}
\textbf{TURN 0: Initial Attempt (Failure)}
\begin{itemize}
    \item \textbf{Solver Strategy:} Coordinate Geometry (Shoelace Formula).
    \item \textbf{Code Output:} The code calculated an area of \textbf{588}.
    \item \textbf{Solver Hallucination:} The Solver effectively guessed the answer was the same as the total area, justifying it with a spatial impossibility.
    \item \textbf{Solver Text Reasoning:} ``Looking at the problem again, it's clear that the heptagon AFNBCEM is the entire triangle ABC. Therefore, the area is indeed 588.''
    \item \textbf{Final Answer (Turn 0):} 588.
\end{itemize}

\vspace{0.5em}
\textbf{VERIFICATION (The Critique)}
\begin{itemize}
    \item \textbf{Verifier Verdict:} \texttt{INVALID}
    \item \textbf{Error Type:} \texttt{LOGIC\_FLAW}
    \item \textbf{Critique:} ``The solver's reasoning is flawed because they incorrectly concluded that the heptagon AFNBCEM has the same area as the entire triangle ABC. The heptagon should be a part of the triangle, not the whole triangle. The solver's repeated use of the shoelace formula without correctly identifying the vertices led to a logical error.''
\end{itemize}

\vspace{0.5em}
\textbf{OUTCOME (Loop Exhaustion)}
\begin{itemize}
    \item \textbf{Failure to Recover:} Because the Planner lacks a Vision-Language Model (VLM), it could not "show" the Solver why the heptagon was a subset. The Solver spent the next 3 turns randomly perturbing coordinates before failing.
    \item \textbf{Note:} This is a ``Right Answer, Wrong Reason'' failure. The ground truth \textit{was} 588, but the Solver's claim that ``Heptagon = Triangle'' was semantically false (the Triangle area was likely larger in the problem's implicit geometry), causing the Verifier to rightly reject the reasoning.
\end{itemize}
